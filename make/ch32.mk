...
CH32FUN?=$(abspath $(CURDIR)/build-tools/ch32fun)
TARGET_EXT?=c
MINICHLINK?=$(CH32FUN)/../minichlink

WRITE_SECTION?=flash
UPLOAD_PORT?=/dev/ttyACM0
FLASH_COMMAND := $(MINICHLINK)/minichlink -w $(if $(TARGET),$(TARGET),index).bin $(WRITE_SECTION) -b -c $(UPLOAD_PORT)
FLASH_EXT_COMMAND := $(MINICHLINK)/minichlink -w $(if $(TARGET),$(TARGET),index)_ext.bin $(EXT_ORIGIN) -b -c $(UPLOAD_PORT)
...
CFLAGS?=-g -Os -ffunction-sections -fdata-sections -fmessage-length=0 -msmall-data-limit=8 -fno-tree-loop-distribute-patterns
...
ifeq ($(findstring CH32V00,$(TARGET_MCU)),CH32V00)
	TARGET_MCU_PACKAGE?=$(TARGET_MCU)
	TARGET_MCU_MEMORY_SPLIT?=0
	MCU_PACKAGE?=1
	GENERATED_LD_FILE?=$(abspath $(CH32FUN)/generated_ch32v003.ld)
...
else ifeq ($(findstring CH32V10,$(TARGET_MCU)),CH32V10)
	TARGET_MCU_PACKAGE?=CH32V103R8T6
	CFLAGS_ARCH+=	-march=rv32imac -mabi=ilp32 -DCH32V10x=1
	...
	TARGET_MCU_LD:=1
else ifeq ($(findstring CH32X03,$(TARGET_MCU)),CH32X03)
	TARGET_MCU_PACKAGE?=CH32X035F8U6
	CFLAGS_ARCH+=-march=rv32imac -mabi=ilp32 -DCH32X03x=1
	...
	TARGET_MCU_LD:=4
else ifeq ($(findstring CH32L103,$(TARGET_MCU)),CH32L103)
	TARGET_MCU_PACKAGE?=CH32L103C8T6
	CFLAGS_ARCH+=-march=rv32imac -mabi=ilp32 -DCH32L103=1
	ifeq ($(findstring F8, $(TARGET_MCU_PACKAGE)), F8)
		MCU_PACKAGE:=1
	else ifeq ($(findstring K8, $(TARGET_MCU_PACKAGE)), K8)
		MCU_PACKAGE:=1
	...
	CFLAGS_ARCH+=	-march=rv32imac -mabi=ilp32 -DCH32V20x=1
	...
	CFLAGS_ARCH+= -DCH32V30x=1 -DTARGET_MCU_MEMORY_SPLIT=$(TARGET_MCU_MEMORY_SPLIT)
	...
	TARGET_MCU_LD:=3
else ifeq ($(findstring CH57,$(TARGET_MCU)),CH57)
	TARGET_MCU_PACKAGE?=CH570E
	CFLAGS_ARCH+=-march=rv32imac -mabi=ilp32 -DCH57x=1 -DCH5xx
	...
	TARGET_MCU_LD:=10
	IS_CH5XX:=1
else ifeq ($(findstring CH58,$(TARGET_MCU)),CH58)
	TARGET_MCU_PACKAGE?=CH582F
	CFLAGS_ARCH+=-march=rv32imac -mabi=ilp32 -DCH58x=1 -DCH5xx
	...
	TARGET_MCU_LD:=8
	IS_CH5XX:=1
else ifeq ($(findstring CH59,$(TARGET_MCU)),CH59)
	TARGET_MCU_PACKAGE?=CH592F
	CFLAGS_ARCH+=-march=rv32imac -mabi=ilp32 -DCH59x=1 -DCH5xx
...
GENERATED_LD_FILE:=$(abspath $(CH32FUN)/generated_$(TARGET_MCU_PACKAGE)_$(TARGET_MCU_MEMORY_SPLIT).ld)
LINKER_SCRIPT?=$(GENERATED_LD_FILE)

CFLAGS+= $(CFLAGS_ARCH) -I$(NEWLIB) -I$(CH32FUN)/../extralibs -I$(CH32FUN) -nostdlib -I. -Wall $(EXTRA_CFLAGS)

LDFLAGS+=-T $(LINKER_SCRIPT) -Wl,--gc-sections

C_FLAGS += $(CFLAGS) $(CFLAGS_ARCH) $(EXTRA_CFLAGS)
CPP_FLAGS += $(CFLAGS) $(CFLAGS_ARCH) $(EXTRA_CFLAGS)
ASM_FLAGS += $(CFLAGS_ARCH)

EXTRA_SOURCES := $(SYSTEM_C) $(CH32FUN)/udiv_mod.c $(ADDITIONAL_C_FILES) $(SOURCE_FILES)
...
TARGET ?= index
EXE_NAME := $(TARGET).elf
$(EXE_NAME): $(LINKER_SCRIPT)

.PHONY: post_link
post_link: $(EXE_NAME)
	@echo "Generating CH32 artifacts..."
...
gdbclient :
	gdb-multiarch $(EXE_NAME) -ex "target remote :3333"
...
FLASH_COMMAND?=$(MINICHLINK)/minichlink -w $< $(WRITE_SECTION) -b -c /dev/ttyACM0
FLASH_EXT_COMMAND?=$(MINICHLINK)/minichlink -w $< $(EXT_ORIGIN) -b -c /dev/ttyACM0
...
cv_clean :
	rm -rf $(EXE_NAME) $(TARGET).bin $(TARGET)_ext.bin $(TARGET).hex $(TARGET).lst $(TARGET).map $(GENERATED_LD_FILE) || true
